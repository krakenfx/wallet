...
export const formatCurrency = (currencyValue: string | number = 0, { currency, hideCurrencySign = false, highPrecision = false, compact = false }: Options) => {
  ...
  if (isSmallNumber) {
    const smallNumberData = getSmallNumberData({ currencyValueNumber, currencyInfo, highPrecision });
  ...
  const formatted = Intl.NumberFormat('en-US', {
    ...
    minimumFractionDigits: currencyInfo.minimumFractionDigits,
...
type SmallNumberOptions = { currencyValueNumber: number; currencyInfo: CurrencyInfo; highPrecision?: boolean };

const getSmallNumberData = ({ currencyValueNumber, currencyInfo, highPrecision }: SmallNumberOptions) => {
  const MAX_FRACTION_DIGITS = 5;
  const tooSmallNumberMaximumFractionDigits = highPrecision ? MAX_FRACTION_DIGITS : 2;
  const tooSmallNumber: number = highPrecision ? 0.00001 : 0.01;
  const isTooSmall = Math.abs(currencyValueNumber) < tooSmallNumber && Math.abs(currencyValueNumber) > 0;
  const isHighPrecisionAndNotTooSmall = highPrecision && !isTooSmall;
  const isNegative = currencyValueNumber < 0;
  const prefix = isTooSmall ? (isNegative ? '-<' : '<') : '';
  const maximumFractionDigits =
    (isHighPrecisionAndNotTooSmall &&
      Math.min(MAX_FRACTION_DIGITS, Math.max(findFirstNonZeroDecimalIndex(String(currencyValueNumber)), currencyInfo.minimumFractionDigits))) ||
