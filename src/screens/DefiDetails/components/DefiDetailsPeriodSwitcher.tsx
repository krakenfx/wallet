...
export const DefiDetailsPeriodSwitcher = ({ onChange, disabled }: Props) => {
  ...
  const handleTap = useCallback(
    (value: number, period: PriceHistoryPeriod) => {
      if (disabled) {
        return;
      }
      setIndex(value);
    ...
    [animation, disabled, setIndex, onChange],
  );

  const renderItem = useCallback(
    (value: number, text: string, period: PriceHistoryPeriod) => {
      return (
        <Touchable
          testID={`DefiDetailsPeriod-${period}`}
          activeOpacity={1}
    ...
    [disabled, handleTap, index, leftWidth],
  );
    ...
    <Animated.View style={[styles.toggleWrapper]} layout={CurvedTransition}>
      <Animated.View style={[styles.container]}>
        ...
        <View style={styles.periods} onLayout={e => (fullWidth.value = e.nativeEvent.layout.width)}>
          ...
          {renderItem(0, loc.earn.detailsSheet.period.day, 'DAY')}
          {renderItem(1, loc.earn.detailsSheet.period.week, 'WEEK')}
          {renderItem(2, loc.earn.detailsSheet.period.month, 'MONTH')}
          {renderItem(3, loc.earn.detailsSheet.period.year, 'YEAR')}
          {renderItem(4, loc.earn.detailsSheet.period.all, 'ALL')}
        </View>
      </Animated.View>
    </Animated.View>
  );
